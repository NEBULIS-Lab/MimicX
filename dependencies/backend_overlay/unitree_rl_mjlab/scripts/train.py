...
def run_train(task_id: str, cfg: TrainConfig, log_dir: Path) -> None:
  selected_ids = os.environ.get("MIMICX_TRAIN_DEVICE_IDS", "")
  if selected_ids == "":
    device = "cpu"
    seed = cfg.agent.seed
    rank = 0
    ...
    device = f"cuda:{selected_ids.split(',')[local_rank]}"
  ...
  if is_tracking_task:
    ...
    has_embedded_motions = bool(getattr(motion_cmd, "motion_files", ()))
    if not cfg.motion_file and not has_embedded_motions:
    ...
    if has_embedded_motions:
      print(f"[INFO] Using {len(motion_cmd.motion_files)} embedded motion files.")
...
def launch_training(task_id: str, args: TrainConfig | None = None):
  ...
  # Device IDs are relative to the inherited visibility mask, which is preserved.
  if "MIMICX_DEVICE_ID" in os.environ:
    selected_gpus = [int(os.environ["MIMICX_DEVICE_ID"])]
  elif args.gpu_ids == "all":
    selected_gpus = list(range(torch.cuda.device_count()))
  else:
    selected_gpus = list(args.gpu_ids or [])
  if any(index < 0 or index >= torch.cuda.device_count() for index in selected_gpus):
    raise ValueError("Requested GPU index is outside the visible device set")
  if len(set(selected_gpus)) != len(selected_gpus):
    raise ValueError("GPU indices must be unique")
  num_gpus = len(selected_gpus)
  os.environ["MIMICX_TRAIN_DEVICE_IDS"] = ",".join(map(str, selected_gpus))
  ...
  if num_gpus <= 1:
    ...
    torchrunx.Launcher(
      ...
      copy_env_vars=torchrunx.DEFAULT_ENV_VARS_FOR_COPY + ("MUJOCO*", "MIMICX*"),
