    ...
    if not file_saved:
      ...
      try:
        torch.testing.assert_close(
          robot.data.body_link_lin_vel_w[0, 0], motion_base_lin_vel[0], atol=1e-1, rtol=1e-1
        )
      except Exception as exc:
        warnings.warn(f"motion base linear velocity assertion skipped: {exc}")
      try:
        torch.testing.assert_close(
          robot.data.body_link_ang_vel_w[0, 0], motion_base_ang_vel[0], atol=1e-1, rtol=1e-1
        )
      except Exception as exc:
        warnings.warn(f"motion base angular velocity assertion skipped: {exc}")
